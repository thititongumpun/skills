...
| `mfec-pptx-diagram` | Turning a Mermaid diagram into an editable PowerPoint slide on the MFEC template — including animating a marker along the route, staging captions, and the comparison table and takeaway box around it. Requires [officecli](https://officecli.ai); the layout-check and animation helpers need `python3`. |
