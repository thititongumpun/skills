...
| `n8n-upgrade` | Upgrading a self-hosted docker-compose n8n + `n8nio/runners` stack to a newer release, or rolling one back after an upgrade broke workflows. Map the stack → docs-verified compatibility report (stops for approval; deprecated/removed nodes and env vars get a replacement suggested) → snapshot → bump both tags together → health, active-workflow diff, smoke run → rollback from the snapshot if anything fails. Needs `docker` and `curl`; `jq` for the diff. |
